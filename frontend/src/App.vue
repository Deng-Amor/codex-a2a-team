...
const apiContract = computed(() => {
  const value = selectedDetail.value.api_contract || selectedDetail.value.apiContract || selectedTask.value?.api_contract || workflow.value?.api_contract
  if (value) return typeof value === 'string' ? value : JSON.stringify(value, null, 2)
  return selected.value?.key === 'team-lead' ? 'REST API Contract 尚未交付；Team Lead 完成方案后会在此展示路径、方法、请求、响应与错误码。' : ''
})

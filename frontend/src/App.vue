...
function select(agent, event) {
  ...
  if (card.top - office.top > office.height / 2) top = 16
  top = Math.min(Math.max(16, top), Math.max(16, office.height - 180))
  detailStyle.value = { left:Math.max(16, left)+'px', top:top+'px', right:'auto', bottom:'16px' }
}

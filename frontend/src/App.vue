...
const agents = ref([]), runtimes = ref([]), workflows = ref([]), workflow = ref(null), workflowId = ref(''), followLatest = ref(true), tasks = ref([]), messages = ref([]), selected = ref(null), error = ref(''), floor = ref(null), detailStyle = ref({}), menuOpen = ref(false), acceptanceReason = ref('')
...
const workflowRuntimes = computed(() => runtimes.value.filter(runtime => runtime.current_workflow_id === workflow.value?.id))
...
const statusText = status => ({ ready:'待分派', queued:'等待真实 Agent 领取', running:'执行中', repairing:'修复已排队', acceptance_pending_human:'等待人工验收', passed:'已交付', blocked:'等待依赖', skipped:'已跳过', not_applicable:'不适用', waiting:'等待方案' }[status] || status || '等待方案')
const runtimeText = state => ({ idle:'在线待命', working:'执行中', starting:'启动中', offline:'离线' }[state] || state)
...
async function load() {
  try {
    ...
    runtimes.value = await fetchJson('/api/agent-runtimes')
...
<template>
  <main class="app">
    <header class="top">
      ...
      <div class="metrics"><div><b>{{ complete }}/{{ tasks.length }}</b><small>已完成任务</small></div><div><b>{{ workflowRuntimes.filter(item => item.state === 'working').length }}</b><small>当前任务真实 Agent</small></div><div><b>{{ workflow?.status?.toUpperCase() || 'IDLE' }}</b><small>工作流状态</small></div></div>
    ...
    <div class="layout">
      <section ref="floor" class="floor" :class="{ 'has-detail': selected }" @click="selected = null">
        <span class="floor-title">任务 DAG · 这里只显示计划任务，不代表 Agent 已启动</span>
        ...
        <article v-for="(agent, index) in displayAgents" :key="agent.key" class="agent" :class="{ active:selected?.key === agent.key }" :style="{ ...position(agent,index), '--accent':color(agent) }" @click.stop="select(agent,$event)">
          <div class="agent-head"><i class="avatar">{{ label(agent)[0] }}<em :class="currentTask(agent)?.status"></em></i><div><b>任务 · {{ label(agent) }}</b><span>责任角色：{{ agent.agentKey || agent.key }}</span></div></div>
      ...
      <aside class="side"><div class="side-head"><b>当前任务的真实 Agent</b><b>{{ workflowRuntimes.length }}</b></div><div v-if="workflowRuntimes.length" class="runtime-list"><article v-for="runtime in workflowRuntimes" :key="runtime.id" class="runtime" :class="runtime.state"><div><b>{{ runtime.agent_key }}</b><em>{{ runtimeText(runtime.state) }}</em></div><p>{{ runtime.adapter }} · {{ runtime.worker_id }}</p><small>Job：{{ runtime.current_job_id }}</small></article></div><div v-else class="empty">当前任务尚无真实 Agent Runtime。<br>计划任务不会被视为正在执行的 Agent。</div><div class="side-head"><b>共享上下文 / 协作日志</b><b>{{ logs.length }}</b></div><div v-if="logs.length" class="feed"><article v-for="message in logs" :key="message.id" class="event"><time>{{ date(message.created_at) }}</time><div><b :style="{ background: color(agents.find(agent => agent.key === messageFrom(message)) || { key:messageFrom(message) }) }">{{ label(agents.find(agent => agent.key === messageFrom(message)) || { key:messageFrom(message), name:messageFrom(message) }) }}</b><span>→</span><b class="archive">{{ label(agents.find(agent => agent.key === messageTo(message)) || { key:messageTo(message), name:messageTo(message) }) }}</b><em v-if="message.kind">{{ message.kind }}</em></div><p>{{ message.text }}</p></article></div></aside>
    </div>
  </main>
</template>
...
.runtime-list{max-height:210px;overflow:auto;border-bottom:1px solid #dbe3ec}.runtime{padding:10px 17px;border-bottom:1px solid #eff2f5}.runtime:last-child{border-bottom:0}.runtime div{display:flex;align-items:center;justify-content:space-between;gap:8px}.runtime b{font-size:12px}.runtime em{padding:2px 6px;border-radius:9px;background:#eef2f5;color:#64748b;font-size:10px;font-style:normal}.runtime.working em{background:#dcfce7;color:#16724c}.runtime.offline em{background:#fef2f2;color:#b42318}.runtime p,.runtime small{display:block;margin:4px 0 0;color:#778394;font-size:10px}.avatar em.queued{background:#dd9a21}.avatar em.running{background:#16a36e}.avatar em.repairing{background:#dc981f}.avatar em.blocked{background:#94a3b8}.side .feed{flex:1}.side .empty{padding:28px 22px}
</style>

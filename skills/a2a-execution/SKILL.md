...
3. On confirmation, require `A2A_TEAM_HOME` to point to the installed A2A Team directory. Confirm FastAPI is listening on `8010` and Dashboard/Vite is listening on `20002`, then run `$env:A2A_TEAM_HOME\start-workflow.ps1`. Pass the confirmed scope as `Request` and the project root as `Repository`.
